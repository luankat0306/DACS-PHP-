...
<link rel="stylesheet" type="text/css" href="../CSS/Search.php">
    ...
    <h2 class="h2"style="color:#2196F3" >TÌM KIẾM THEO TÊN CÔNG TY</h2>
    <?php if(isset($alert)):?>        
    ...
    <hr>
<div class="box">
    <h3 style="color:#2196F3" class="otherh">Nhập tên công ty để tìm kiếm</h3>
    <form class="example" action="" method = "GET" >
        <input class="form-control" type="text" placeholder="Tìm Kiếm..." name="search" value="<?php echo $search; ?>">
        <button class="corp" type="submit" name = "submit-search"><i class="fa fa-search" ></i></button>
    </form>
</div>
...
/*************************** SHOW JOB **************************** */
    if (isset($_GET['submit-search'])):
        ...
        $sql =("SELECT * FROM congty INNER JOIN dangtincongviec ON congty.id_CongTy = dangtincongviec.id_CongTy  INNER JOIN nganhnghe ON nganhnghe.id_NganhNghe = dangtincongviec.id_NganhNghe WHERE TenCongTy LIKE '%$search%' ");
        $query = $connect->query($sql) or die($connect -> error());
        if($query->num_rows > 0): 
?>
    <div class="searchbox">
