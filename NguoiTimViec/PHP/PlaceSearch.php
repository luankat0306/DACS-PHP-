...
<link rel="stylesheet" type="text/css" href="../CSS/Search.php">
    ...
    <h2 class="h2" style="color:#e74a43">TÌM KIẾM THEO TỈNH THÀNH</h2>
    <?php if(isset($alert)):?>        
    ...
    <hr>
<div class="box">
    <h3 style="color:#e74a43" class="otherh">Nhập tỉnh thành để tìm kiếm</h3>
    <form class="example" action="" method="GET">
        <input class="form-control" type="text" placeholder="Tìm Kiếm..." name="search">
        <button class="place" type="submit" name="submit-search"><i class="fa fa-search"></i></button>
    </form>
</div>
<?php
/*************************** SHOW INFO **************************** */
    if (isset($_GET['submit-search'])):
        ...
        $sql =("SELECT * FROM congty INNER JOIN dangtincongviec ON congty.id_CongTy = dangtincongviec.id_CongTy  INNER JOIN nganhnghe ON nganhnghe.id_NganhNghe = dangtincongviec.id_NganhNghe WHERE TinhThanh LIKE '%$search%' ");
        $query = $connect->query($sql) or die($connect -> error());
        if($query->num_rows > 0): 
?>
    <div class="searchbox">
